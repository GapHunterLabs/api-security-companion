package dev.gaphunter.apisecuritycompanion.licensing
...
/**
 * Ported verbatim from JetBrains's own reference implementation
 * (github.com/JetBrains/marketplace-makemecoffee-plugin,
 * src/main/java/com/company/license/CheckLicense.java) -- the same port
 * already used in `ansible-companion` (`PANSIBLECOMPANI`, a real
 * assigned code) and `refactor-simulator` (a placeholder, not yet
 * enrolled). The certificate/crypto logic below is JetBrains's own
 * boilerplate for verifying a license was genuinely signed by them --
...
object CheckLicense {
    ...
    private fun showRegisterDialog(productCode: String, message: String?) {
        ...
        if (registerAction != null) {
            // Same API confirmed by ansible-companion's own port (see its
            // CheckLicense.kt for the full trail of two other candidates
            // confirmed wrong by real verifyPlugin runs): invokeAction is
            // ActionUtil's own documented replacement for the deprecated
            // Component-based overload, accepts DataContext, and isn't
            // flagged Experimental/OverrideOnly/Internal.
